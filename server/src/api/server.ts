...
export class DashboardServer {
  ...
  private githubMonitor: GitHubMonitor | null = null;
  private failureDetector: FailureDetector | null = null;
  private currentFailures: import('../github/FailureDetector.js').DetectedFailure[] = [];
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/api/github/failures', async (_req, res) => {
      ...
      try {
        ...
        const checkRunsMap = new Map<string, import('../github/GitHubMonitor.js').GitHubCheckRun[]>();
        ...
        for (const issue of issues) {
          if (issue.pull_request) {
            try {
              ...
              console.warn(`[GitHub] Failed to fetch check runs for PR #${issue.number}:`, (err as Error).message);
            }
          }
        }
  ...
  private setupWebSocket(): void {
    this.wss.on('connection', (ws) => {
      ...
      const unsubscribeEvent = paperclipAnalyzer.on('event', (event: unknown) => {
        ws.send(JSON.stringify({
          type: 'event',
          data: event,
        }));
      ...
      const unsubscribeAgent = paperclipAnalyzer.on('heartbeat.run.status', (payload: unknown) => {
        ws.send(JSON.stringify({
          type: 'agent_update',
          data: payload,
        }));
  ...
  private broadcastFailures(): void {
    if (this.currentFailures.length === 0) return;
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      console.warn('[DashboardServer] Could not connect to Paperclip:', (err as Error).message);
