...
export class AgentMonitor {
  private runLogsPath: string;

  constructor(projectRoot: string) {
    // Default Paperclip run logs path
    this.runLogsPath = path.join(
  ...
  private async analyzeAgentRuns(agentPath: string, agentId: string): Promise<AgentActivity | null> {
    try {
        ...
        try {
          ...
          for (const line of lines) {
            try {
              ...
              if (entry.chunk) {
                const chunk = entry.chunk as string;
  ...
  private getMockData(): { agents: AgentActivity[]; events: ProjectEvent[] } {
    const agents: AgentActivity[] = [
      {
        agentId: 'agent-1',
        agentName: 'CEO',
