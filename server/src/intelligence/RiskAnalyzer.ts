...
export class RiskAnalyzer {
  ...
  async analyze(): Promise<{ risks: ModuleRisk[]; events: ProjectEvent[] }> {
  ...
  private async assessFileRisk(filePath: string): Promise<ModuleRisk> {
    try {
      ...
      if (lines > 300) riskScore += 15;
      else if (lines > 200) riskScore += 10;
      else if (lines > 100) riskScore += 5;
      ...
      if (dbQueries > 0 && authChecks === 0) riskScore += 15;
